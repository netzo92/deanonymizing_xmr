...
- [ ] **D14 — Dated TODO activity and hypothesis outcomes.** Show when each task was first recorded, closed, reopened or removed, with Git provenance and an activity feed that highlights new published events. Keep imported completed tasks distinct from observed closures. For every research checklist, expose the associated claim or engineering objective, execution status, scientific verdict, theoretical implication, measured result, evidence scope and remaining work. Done when source/task coverage checks pass, the ledger preserves untested and deferred hypotheses, refresh retains user context, and desktop/mobile/publication checks pass. New chain data must not automatically close checklists or validate hypotheses.
