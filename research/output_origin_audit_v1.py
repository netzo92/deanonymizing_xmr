    ...
    for category, predicate in FILTERS.items():
        ...
        for priority, amount, index, key_image in sorted(heap, reverse=True):
            member_rows = conn.execute(
                "SELECT rm.amount, rm.global_output_index, rm.tx_hash, t.block_height "
            ...
            tx_hash, input_height = member_rows[0][2:]
...
def main():
    ...
    if source == target:
        parser.error("Output cannot replace the source database")
    started_at = datetime.now(timezone.utc).isoformat()
    before = sha256_file(source)
    with sqlite3.connect(source.as_uri() + "?mode=ro", uri=True, timeout=30) as conn:
    ...
    report = {
        "audit_version": 1, "started_at": started_at,
        "finished_at": datetime.now(timezone.utc).isoformat(),
        "source_revision": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(),
        "script_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_sha256": sha256_file(ROOT / "scorer.py"),
        "database_file": source.name, "database_sha256_before": before,
        "database_sha256_after": after, "source_file_unchanged": before == after,
    ...
    print(json.dumps({"summary": summary, "errors": errors,
                      "source_file_unchanged": before == after, "rpc_elapsed_seconds": elapsed}, indent=2))
